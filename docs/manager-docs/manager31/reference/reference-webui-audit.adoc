...
Select menu:Audit[]
 from the left navigation menu to audit your managed systems. 

[[_ref.webui.audit.cve]]
== CVE Audit
(((CVE audit,{susemgr} data)))


The menu:CVE Audit[]
 page will display a list of client systems with their patch status regarding a given CVE (Common Vulnerabilities and Exposures) number. 

.
image::audit_cve_audit.png[]
...
. Make sure that the CVE data is up-to-date. For more information, see <<_sec_cve_maint>> . 
. Click menu:Audit[CVE Audit] in the left navigation menu to open the menu:CVE Audit[] page. 
. Input a 13-char CVE identifier in the menu:CVE Number[] field. The year setting will be automatically adjusted. Alternatively, set the year manually and add the last four digits. 
. Optionally, uncheck the patch statuses you are not interested in. 
. Click menu:Audit systems[] . 


Performing this procedure will result in a list of client systems, where each system comes with a menu:Patch Status[]
 belonging to the given CVE identifier.
Possible statuses are: 

image:patch-status-affected-patch-inapplicable.png[[red]] -- Affected, patches are available in channels that are not assigned:::
...
image:patch-status-affected-patch-applicable.png[[orange]] -- Affected, at least one patch available in an assigned channel:::
...
image:patch-status-not-affected.png[[grey]] -- Not affected:::
The system does not have any packages installed that are patchable. 

image:patch-status-patched.png[[green]] -- Patched:::
...
** The image:patch-status-affected-patch-applicable.png[[orange]] state is displayed when {susemgr} has at least one patch in an assigned channel. This might mean that, after installing such patch, others might be needed{mdash}users should double check the CVE Audit page after applying a patch to be sure that their systems are not affected anymore. 


For a more precise definitions of these states, see <<_sec_cve_tips>>
. 
...
For each system, the menu:Next Action[]
...
If applicable, a list of "`candidate`"
 channels or patches is displayed for your convenience. 

You can also assign systems to a menu:System Set[]
...
.. In the Web interface, click menu:Admin[Task Schedules] in the left navigation menu. 
.. Click the `cve-server-channels-default` schedule link. 
.. Click the `cve-server-channels-bunch` link. 
.. Click the menu:Single Run Schedule[] button. 
.. After some minutes, refresh the page and check that the scheduled run status is ``FINISHED``. 


A direct link is also available on the menu:CVE Audit[]
 page (menu:extra CVE data update[]
). 
...
Audit results are only correct if the assignment of channels to systems did not change since the last scheduled refresh (normally at 11:00 PM every night). If a CVE audit is needed and channels were assigned or unassigned to any system during the day, a manual run is recommended.
For more information, see <<_sec_cve_maint>>
. 

Systems are called "`affected`"
, "`not affected`"
 or "`patched`"
 not in an absolute sense, but based on information available to {susemgr}. This implies that concepts such as "`being
...
[[_ref.webui.audit.subscription]]
== menu:Subscription Matching[]


To match subscriptions with your systems use the Subscription Matcher tool.
For more information about Virtual Host Managers, see <<_ref.webui.audit.submatch.vhm>>
. 

.
image::audit_subscription_matching.png[]
...
*** The menu:Subscriptions Report[] provides subscriptions report data when used 
*** The menu:Unmatched Products Report[] provides information on products and their systems when a match to a subscription cannot be found 
*** The menu:Error Report[] provides a list of errors raised during the matching process 


Selecting menu:Audit[Subscription
   Matching]
 from the left navigation menu will provide you with an overview of all results generated by the Subscription Matcher. 
...
=== menu:Subscription Matching[]{gt} menu:Subscriptions[]
...
.Fully Matched
If the total amounts of a subscription are fully matched, the quantity column value is highlighted with a yellow warning triangle: image:fa-warning.svg[Warning]
*** {empty}
+
...
These reports are stored in [path]_/var/lib/spacewalk/subscription-matcher_
 and provided in CSV format.
These CSV files may be opened with a spreadsheet application such as LibreOffice Calc. 

If you want to schedule these reports to be produced at different times, or at a certain frequency or schedule a one time completion you can perform this task by editing the Taskomatic settings for the gatherer-matcher located in the {webui} at menu:Admin[Task Schedules > gatherer-matcher-default]
. 

.
image::audit_subscription_matching_gatherer_matcher_default.png[]


[[_ref.webui.audit.submatch.unmatched.systems]]
=== menu:Subscription Matching[]{gt} menu:Unmatched Products[]


Selecting the menu:Subscription Matching[Unmatched Products]
 tab provides an overview of all systems the matcher could not find in the database or which were not registered with {susemgr}. The menu:Unmatched
    Products[]
 overview contains product names and the number of systems which remain unmatched with known installed products. 

.Unmatched Products
image::audit_subscription_matching_unmatched_products.png[]
...
[[_ref.webui.audit.submatch.sub.pinning]]
=== menu:Subscription Matching[]{gt} menu:Pins[]
...
.
image::audit_subscription_matching_pins.png[]
...
Items in the list contain the status of pins, which can be ``satisfied``, `not
    satisfied` and ``pending next run``. 
*** A pin is `satisfied` if its system and subscription was matched in the last matcher run. 
*** A pin is `not satisfied` if its system and subscription was _not_ matched in the last matcher run. This can happen, for example, if the pin violates terms and conditions for subscriptions. 
...
Click the menu:Add a Pin[]
 button to open the menu:Available Systems[]
 window.
...
Within the menu:Subscriptions Available for Selected System[]
 window click the menu:Save Pin[]
 button to raise priority for subscription use on the selected system. 

[[_ref.webui.audit.submatch.submatch.messages]]
=== menu:Subscription Matching[]{gt} menu:Messages[]


You can review all messages related to menu:Subscription
    Matching[]
...
.
image::audit_subscription_matching_messages.png[]


[[_ref.webui.audit.submatch.vhm]]
=== Virtual Host Managers


A pre-requirement for matching subscriptions with systems is to have complete information covering which virtual systems are running on which virtual host.
{susemgr} gets this information without any further configuration if the hypervisor is a registered system running on {sls} or {rhel}. Third party hypervisors, like VMware or Hyper-V, will need further configuration as described in this chapter. 

Third party hypervisors and hypervisor managers such as VMWare vCenter are called "`Virtual Host Managers`"
 (VHM) within {susemgr}, as they can manage one or multiple virtual hosts, which in turn may contain virtual guests.
{susemgr} {productnumber} ships with a tool `virtual-host-gatherer` that can connect to VHMs using their API, and request information about virtual hosts.
...
On the menu:Subscription Matching[]
 overview select menu:Edit Virtual Host Managers[]
.
This will take you to the menu:Systems[Virtual Host
    Managers]
 overview.
In the upper right you can select either menu:Add VMware-based Virtual Host
    Manager[]
 or menu:Add File-based Virtual Host
    Manager[]
.
These modules allow you to match subscriptions to machines managed by a virtual host such as VMware, ESX, ESXi, and vCenter.
If using an unsupported virtual host you can also create a custom json file to provide `virtual-host-gatherer` with the required host and guest information. 
...
Select menu:Add File-based Virtual Host
     Manager[]
...
Take care that the module parameter map must match the map returned by `virtualhostmanager.getModuleParameters` to work correctly: 
...
[[_ref.webui.audit.openscap]]
== OpenSCAP


If you click menu:Audit[OpenSCAP > All
   Scans]
 in the left navigation menu, an overview of the {openscap} Scans appears.
...
For a description of the {webui} dialogs, see <<_s2_openscap_suma_pages>>
. 

For instructions and tips on how to best use OpenSCAP with {susemgr}, refer to <<_ch_openscap>>
.
To learn more about OpenSCAP, see the project home page at http://open-scap.org. 

:leveloffset: 1
include::manager_reference_openscap.adoc[]
:leveloffset: 0
