...
== Overview
((({susemgr} Administrator,System Overview)))
...
Then, clicking the number of systems takes you to the menu:Systems[]
 tab of the menu:System Group
   Details[]
 page, while clicking the group name takes you to the menu:Details[]
 tab for that system group.
Refer to <<_ref.webui.systems.systemgroups.details>>
 for more information. 

You can also click menu:Use in SSM[]
 in the menu:System
   Groups[]
 section to go directly to the menu:System Set
   Manager[]
.
Refer to <<_ref.webui.systems.ssm>>
 for more information. 

[[_ref.webui.systems.systems]]
== Systems
(((system list)))
((({susemgr} Administrator,viewing a list of)))
((({susemgr} Administrator,status)))
...
-- Critical patch (errata) available, update _strongly_ recommended. 
+ 
...
-- System is being deployed using {ay} or Kickstart. 
+ 
...
[IMPORTANT]
====
If {susemgr} identifies package updates for the system, but the package updater (such as Red Hat Update Agent or {yast}) responds with a message like {quot}Your system is fully updated{quot}, a conflict likely exists in the system's package profile or in the [path]_up2date_
 configuration file.
...
Clicking the host name of such a guest system displays its menu:System
    Details[]
 page. 
...
Non-compliant systems have packages installed which are not available from {susemgr}. menu:Packages[]
...
The menu:Inactive Systems[]
 page displays systems that have not checked in with {susemgr} for 24 hours or more. 
...
If you see a message telling you that check-ins are not taking place, the client system is not successfully connecting to {susemgr}. The reason may be one of the following: 
...
* The system is entitled, but the {susemgr} daemon ([path]_rhnsd_ ) has been disabled on the system. Refer to <<_bp.contact.methods.rhnsd>> for instructions on restarting and troubleshooting. 
* The system is behind a firewall that does not allow connections over [path]_https_ (port 443). 
...
The menu:Proxy[]
 page displays the {susemgrproxy} Server systems registered with your {susemgr} server. 
...
Active entitlements are in gray, while inactive entitlements are highlighted in yellow and their check boxes checked by default for you to delete them as needed by clicking the menu:Delete Selected[]
 button.
Entitlements are inactive if the system has not checked in with {susemgr} in a time specified via the drop-down box menu:A system
    profile is inactive if its system has not checked in for[]
. 
...
You may filter further by inactive time or typing the system's host name, IP address, IPv6 address, or MAC address in the corresponding menu:Filter
    by[]
 text box. 

To compare up to three duplicate entitlements at one time, click the menu:Compare Systems[]
 link in the menu:Last Checked
    In[]
 column.
...
System Types define the set of functionalities available for each system in {susemgr} such as the ability of installing software or creating guest virtual machines. 
...
To change system types, select the systems you want to modify, and choose either menu:Add System
    Type[]
...
When systems are registered to {susemgr}, they are displayed on the menu:Systems[Overview]
 page.
...
If you mistakenly deleted a system profile from {susemgr}, you may re-register the system using the bootstrap script or `rhnreg_ks` manually. 
====
...
For example Traditional systems (Management, <<_fig.system_details.details.overview.trad>>
) and Salt systems (Salt, <<_fig.system_details.details.overview.salt>>
) display differerent tabs. 

.System Details Overview (Traditional Client)
image::system_details_traditional_overview.png[]


.System Details Overview (Salt Minion)
image::system_details_salt_overview.png[]


[[_s5_sm_system_details_overview]]
==== System Details {gt}Details{gt}Overview
...
To apply these updates, click menu:Packages[]
 and select some or all packages to update, then click menu:Upgrade
         Packages[]
. 
====
.menu:System Info[]
...
[NOTE]
====
The system ID can be used to eliminate duplicate profiles from {susemgr}. Compare the system ID listed on this page with the information stored on the client system in the [path]_/etc/sysconfig/rhn/systemid_
 file.
...
To prevent the application of automated patch updates, deselect menu:Auto Patch
Update[]
...
For example, if a user logs in to the client directly and runs {yast} Online Update (on {slea}) or `pup` (on {rhela}), the update tool will install available patches even if the system is locked in the Web interface. 
...
When finished making selections, click the menu:Change
Subscriptions[]
...
An example is the menu:{susemgr} Tools[]
channel. 
...
+
Push enables {susemgr} customers to immediately initiate tasks rather than wait for those systems to check in with {susemgr}. Scheduling actions through push is identical to the process of scheduling any other action, except that the task can immediately be carried out instead of waiting the set interval for the system to check in. 
+
In addition to the configuration of {susemgr}, to receive pushed actions each client system must have the [path]_osad_
...
==== System Details {gt}Details{gt}Properties
...
The {susemgr} Daemon ([path]_rhnsd_
...
To confirm any changes to these fields, click the menu:Update
     Properties[]
 button. 
...
==== System Details {gt}Details{gt}Remote Command
...
. On {slea} clients, subscribe the system to the {susemgr} Tools child channel. Then use Zypper to install the ``rhncfg``, ``rhncfg-client``, and `rhncfg-actions` packages, if not already installed: 
+
...
----
. Log in to the system as root and add the following file to the local {susemgr} configuration directory: [path]_allowed-actions/scripts/run_ . 
+
...
----
** Create an empty [path]_run_ file in that directory to act as a flag to {susemgr}, signaling permission to allow remote commands: 
+
...
==== System Details {gt}Details{gt}Reactivation[Management]
(((reactivating,systems)))


Reactivation keys include this system's ID, history, groups, and channels.
This key can then be used only once with the `rhnreg_ks` command line utility to re-register this system and regain all {susemgr} settings.
Unlike typical activation keys, which are not associated with a specific system ID, keys created here do not show up within the menu:Activation
     Keys[]
 page. 
...
==== System Details {gt}Details{gt}Hardware
...
If the hardware profile looks incomplete or outdated, click the menu:Schedule Hardware
     Refresh[]
 button.
The next time the {susemgr} Daemon ([path]_rhnsd_
) connects to {susemgr}, it will update your system profile with the latest hardware information. 

[[_s5_sm_system_details_migrate]]
==== System Details {gt}Details{gt}Migrate


This subtab provides the option to migrate systems between organizations.
Select an menu:Organization Name[]
 and click menu:Migrate
     System[]
...
==== System Details {gt}Details{gt}Notes
...
To modify a note, click its subject in the list of notes, make your changes, and click the menu:Update[]
 button.
To remove a note, click its subject in the list of notes then click the menu:delete
     note[]
 link. 
...
==== System Details {gt}Details{gt}Custom Info
...
Click the name of the key in the resulting list and enter a value for it in the menu:Description[]
 field, then click the menu:Update
     Key[]
 button. 

[[_s5_sm_system_details_proxy]]
==== System Details {gt}Details{gt}Proxy[Proxy]
...
==== System Details{gt}Software{gt}Patches


This subtab contains a list of patch (errata) alerts applicable to the system.
Refer to <<_ref.webui.intro.patch_alerts>>
...
To apply updates, select them and click the menu:Apply
     Patches[]
 button.
...
Clicking a status other than `None` takes you to the menu:Action Details[]
 page.
This column corresponds to the one on the menu:Affected
     Systems[]
 tab of the menu:Patch Details[]
 page. 

[[_s4_sm_system_details_packages]]
==== System Details {gt}Software{gt}Packages
...
The next time the {susemgr} daemon ([path]_rhnsd_
) connects to {susemgr}, it updates your system profile with the latest list of installed packages. 
...
Search for the wanted packages by typing its name in the menu:Filter by Package
Name[]
...
page.
To upgrade packages immediately, select them and click the menu:Upgrade
Packages[]
button.
...
page.
To install packages, select them and click the menu:Install
Selected Packages[]
button.
...
button.
These profiles are kept within the menu:Stored
Profiles[]
...
To remove specific packages installed on the system, select the check boxes of these packages showing a difference of menu:This
System Only[]
.
...
==== System Details {gt}Software{gt}Software Channels
...
Click a channel name to view its menu:Channel Details[]
 page.
To modify the child channels associated with this system, use the check boxes next to the channels and click the menu:Change
     Subscriptions[]
 button.
You will receive a success message or be notified of any errors. In case of Salt minions, do not forget applying the high state; for more information, see <<_s5_sm_system_details_states_highstate>>
. 
...
==== System Details {gt}Software{gt}SP Migration
...
.Procedure: Performing a Migration... From the menu:Systems Overview[] page, select a client. 
...
=== System Details {gt}Configuration[Management]
...
To manage the configuration of a system, it must have the latest [package]#rhncfg*#
 packages installed.
Refer to <<_ref.webui.config.preparing>>
...
==== System Details{gt}Configuration{gt}Overview
...
==== System Details {gt}Configuration{gt}View/Modify Files
...
.
image::system_details_traditional_configuration_view_mod_central_paths.png[]Centrally-managed configuration files are provided by global configuration channels.
...
column to find out if any files are overridden, or click menu:Override this
file[]
...
.
image::system_details_traditional_configuration_view_mod_local_paths.png[]Locally-managed configuration files are useful for overriding centrally-managed configuration profiles that cause problems on particular systems.
...
.
image::system_details_traditional_configuration_view_mod_sandbox_paths.png[]In the sandbox you can store configuration files under development.
...
==== System Details {gt}Configuration{gt}Add Files


To upload, import, or create new configuration files, click menu:Add
     Files[]
. 
...
.
image::system_details_traditional_configuration_add_files_upload.png[]To upload a configuration file from your local machine, browse for the upload file, specify whether it is a text or binary file, enter menu:Filename/Path[]
and user and group ownership.
Specific file permissions can be set.
When done, click menu:Upload
Configuration File[]
. 
...
.
image::system_details_traditional_configuration_add_files_import.png[]Via the menu:Import Files[]
...
enter the full path of any files you want import into {susemgr} or select deployable configuration files from the menu:Import Existing
Files[]
list.
When done, click menu:Import Configuration
Files[]
. 
...
.
image::system_details_traditional_configuration_add_files_create.png[]Under menu:Create File[]
...
When done, click menu:Create
Configuration File[]
. 

[[_s5_sdc_configuration_deploy_files]]
==== System Details {gt}Configuration{gt}Deploy Files
...
Files from configuration channels with a higher priority take precedence over files from configuration channels with a lower priority. ##emap 2014-04-22: can't
      see if the rest is still accurate or has changed.#

[[_s5_sdc_configuration_diff]]
==== System Details {gt}Configuration{gt}Compare Files
...
Select the files to be compared, click the menu:Compare
     Files[]
...
After the diff has been performed, go to menu:Recent Events[]
 in <<_s5_sdc_configuration_overview>>
 to see the results. 

[[_s5_sdc_configuration_channels]]
==== System Details{gt}Configuration{gt}Manage Configuration Channels
...
For example, the higher-ranked channel may have an [path]_httpd.conf_
 file that will take precedence over the same file in a lower-ranked channel. 

[[_s4_sm_system_details_kick]]
=== System Details {gt}Provisioning[Management]


The menu:Provisioning[]
 tab and its subtabs allow you to schedule and monitor {ay} or Kickstart installations and to restore a system to its previous state. 
...
==== System Details{gt}Provisioning{gt}Autoinstallation
...
If you have not created any profiles, refer to <<_ref.webui.systems.autoinst.profiles.create>>
 before scheduling an autoinstallation for a system. 
====


To alter autoinstallation settings, click the menu:Advanced
     Configuration[]
 button.
...
Alternatively, click menu:Create PXE Installation
     Configuration[]
...
For more information about Cobbler and {kickstart} templates, refer to <<_advanced.topics.cobbler>>
. 

[[_s5_sdc_provisioning_powermgnt]]
==== System Details{gt}Provisioning{gt}Power Management
...
To use any power management functionality, IPMI configuration details must be added to {susemgr}. First select the target system on the systems list, then select menu:Provisioning[Power
     Management]
.
...
.... When all configuration parameters are set correctly, click menu:Manage[] , then menu:Provisioning[Power Management Operations] to power on, off or reboot systems from the set. Provisioning entitlement is required for non-bare metal systems. 
...
Those are not supported by {susemgr} but can be used by adding the fence agent names as a comma-separated list to the `java.power_management.types` configuration parameter. 

[[_s4_sm_system_details_shots]]
==== System Details{gt}Provisioning{gt}Snapshots


Snapshots enable you to roll back the system's package profile, configuration files, and {susemgr} settings. 
...
***** It is possible to disable snapshotting globally (in [path]_rhn.conf_ set ``enable_snapshots = 0``), but it is enabled by default. No further fine tuning is possible. 
...
When satisfied with the reversion, return to the menu:Rollback[]
 subtab and click the menu:Rollback to
     Snapshot[]
 button.
To see the list again, click menu:Return to
     snapshot list[]
. 
...
==== System Details {gt}Provisioning{gt}Snapshot Tags
...
To tag the most recent snapshot, click menu:Create System
     Tag[]
...
=== System Details {gt}Virtualization
...
==== System Details {gt} Virtualization{gt} Details
...
[NOTE]
====
If a guest system has not registered with {susemgr}, this information appears as plain text in the table. 
====


If you have System Group Administrator responsibilities assigned for your guest systems, a user might see the message menu:You do not have
    permission to access this system[]
 in the table.
...
=== System Details {gt}Groups
((({susemgr} Administrator,assigning and removing)))
...
==== System Details{gt}Groups{gt}List/Leave
...
Only System Group Administrators and {susemgr} Administrators can remove systems from groups.
Non-admins see a menu:Review this system's group
     membership[]
 page.
...
==== System Details {gt}Groups{gt}Join
...
Only System Group Administrators and {susemgr} Administrators can add a system to groups.
Non-admins see a menu:Review this system's group
     membership[]
 page.
...
=== System Details {gt}Audit[Management]
...
=== System Details {gt}States[Salt]
(((system states)))
((({susemgr} Administrator,System states)))
...
==== System Details {gt}States{gt}Packages
...
==== System Details {gt}States{gt}Custom
...
==== System Details {gt}States{gt}Highstate
...
Then click menu:Apply
     Highstate[]
. 

[[_s4_sm_system_details_formualas]]
=== System Details {gt}Formulas[Salt]
...
==== System Details{gt}Events{gt}Pending
...
==== System Details {gt}Events{gt}History
...
To view details of an event, click its summary in the menu:System
     History[]
 list.
...
== System Groups
(((system group)))
((({susemgr} Administrator,list of)))
((({susemgr} Administrator,System Group List)))
(((Web Interface,System Groups)))


The menu:System Groups[]
 page allows {susemgr} users to view the menu:System Groups[]
 list. 
...
Only System Group Administrators and {susemgr} Administrators may perform the following additional tasks: 
...
((({susemgr} Administrator,adding and removing)))
...
Select the systems and click the menu:Add
    Systems[]
 button. 
...
Clicking menu:Work With
    Group[]
 loads the group's systems and launches the menu:System
    Set Manager[]
...
==== System Group Details{gt}Details
((({susemgr} Administrator,deleting)))
...
==== System Group Details {gt}Systems


Lists all members of the system group.
Clicking links within the table takes you to corresponding tabs within the menu:System
     Details[]
...
Clicking it does not delete systems from {susemgr} entirely.
This is done through the menu:System Set Manager[]
 or menu:System
     Details[]
 pages.
Refer to <<_ref.webui.systems.ssm>>
 or <<_s3_sm_system_details>>
, respectively. 

[[_s4_sm_system_group_details_target]]
==== System Group Details {gt}Target Systems
...
==== System Group Details {gt}Patches
...
(Refer to <<_s3_sm_errata_details>>
 for more information.) Clicking the Affected Systems number lists all of the systems affected by the patch.
...
==== System Group Details {gt}States[Salt]
...
.Procedure: Applying States at the Group Level..... Create a state using the menu:Salt[State Catalog] or via the command line. 
...
=== System Set Manager {gt}Overview
...
=== System Set Manager {gt}Packages
...
==== System Set Manager {gt}Packages{gt}Install
...
==== System Set Manager {gt}Packages{gt}Remove
...
==== System Set Manager {gt}Packages{gt}Upgrade
...
[[_s3_sm_ssm_verify]]
==== System Set Manager {gt}Packages{gt}Verify
...
Select the check box next to all packages to be verified, then click the menu:Verify Packages[]
 button.
On the next page, select a date and time for the verification, then click the menu:Schedule
     Verifications[]
 button. 

[[_ref.webui.systems.ssm.groups]]
=== System Set Manager {gt}Groups
...
These functions are limited to {susemgr} Administrators and System Group Administrators.
To add a new group, click menu:Create Group[]
 on the top-right corner.
In the next page, type the group name and description in the respective fields and click the menu:Create
    Group[]
 button.
...
=== System Set Manager {gt}Channels


Manage channel associations through the following subtabs: 

[[_s4_sm_ssm_channels_base]]
==== System Set Manager{gt}Channels{gt}Base Channels
...
To change the base channel, select the new one from the menu:Desired
     base Channel[]
 and confirm the action. 

[[_s4_sm_ssm_channels_child]]
==== System Set Manager {gt}Channels{gt}Child Channels


The menu:Child Channels[]
 tab allows you to subscribe and unsubscribe individual child channels related to its parent or base channel. 

.
image::systems_ssm_child_subscriptions.png[]


To subscribe or unsubscribe selected systems to any of the channels, toggle the appropriate check boxes and click the menu:Alter
     Subscriptions[]
 button.
Keep in mind that subscribing to a channel uses a channel entitlement for each system in the selected group.
If too few entitlements are available, some systems fail to subscribe.
Systems must subscribe to a base channel before subscribing to a child channel. 

[[_ref.webui.systems.ssm.config]]
=== System Set Manager {gt}Configuration
...
The channels are created in the menu:Manage Config
    Channels[]
 interface within the menu:Channels[]
 category.
Refer to <<_ref.webui.config.overview>>
 for channel creation instructions. 

To manage the configuration of a system, install the latest [path]_rhncfg*_
 packages.
Refer to <<_ref.webui.config.preparing>>
 for instructions on enabling and disabling scheduled actions for a system. 

[[_ref.webui.systems.ssm.config.deploy]]
==== System Set Manager{gt}Configuration{gt}Deploy Files


Use this subtab to distribute configuration files from your central repository on {susemgr} to each of the selected systems. 
...
==== System Set Manager {gt}Configuration{gt}Compare Files
...
To compare the configuration files deployed on the systems with those in {susemgr}, select the check box for each file to be validated.
Then click menu:Analyze Differences[Schedule File
     Comparison]
.
The comparisons for each system will not complete until each system checks in to {susemgr}. When each comparison is complete, any differences between the files will be accessible from each system's events page. 
...
==== System Set Manager {gt}Configuration{gt}Subscribe to Channels
...
This tab is available only to {susemgr} Administrators and Configuration Administrators. 
...
Clicking menu:Subscribe With Lowest
Priority[]
...
[NOTE]
====
In the first two cases, if any of the newly ranked configuration channels are already in a system's existing configuration channel list, the duplicate channel is removed and replaced according to the new rank, effectively reordering the system's existing channels.
...
==== System Set Manager {gt}Configuration{gt}Unsubscribe from Channels
...
==== System Set Manager {gt}Configuration{gt}Enable Configuration
...
, {susemgr} will automatically pick a profile for each system so that the system's IP address will be in one of the IP ranges specified in the profile itself.
If such a profile cannot be found, {susemgr} will look for an organization default profile and apply that instead. If no matching IP ranges nor organization default profiles can be found, no autoinstallation will be performed on the system.
You will be notified on the next page if that happens. 

To use Cobbler system records for autoinstallation, select menu:Create
...
{susemgr} and its network must be properly configured to enable boot using PXE.
For more information on Cobbler and {kickstart} templates, refer to <<_advanced.topics.cobbler>>
. 
...
If any of the systems connect to {susemgr} via a proxy server, choose either the menu:Preserve Existing Configuration[]
...
When the autoinstallations for the selected systems are successfully scheduled, you will return to the menu:System Set
     Manager[]
 page. 

[[_ref.webui.systems.ssm.provision.tag]]
==== System Set Manager {gt}Provisioning{gt}Tag Systems
...
To tag the most recent system snapshots, enter a descriptive term in the menu:Tag name[]
 field and click the menu:Tag Current
     Snapshots[]
 button. 

[[_ref.webui.systems.ssm.provision.roll]]
==== System Set Manager {gt}Provisioning{gt}Rollback
...
==== System Set Manager {gt}Provisioning{gt}Remote Command
...
First create a [path]_run_
 file on the client systems to allow this function to operate.
Refer to <<_s5_sm_system_details_remote>>
 for instructions.
...
==== System Set Manager {gt}Provisioning{gt}Power Management Configuration
...
==== System Set Manager {gt}Provisioning{gt}Power Management Operation
...
=== System Set Manager {gt}Misc


On the menu:Misc[]
 page, you can modify menu:Custom System
    Information[]
.
Click menu:Set a custom value for selected
    systems[]
, then the name of a key.
Enter values for all selected systems, then click the menu:Set Values[]
 button.
To remove values for all selected systems, click menu:Remove a custom value from
    selected systems[]
...
==== System Set Manager{gt}Misc{gt}Hardware
...
==== System Set Manager {gt}Misc{gt}Software


Click the menu:Software[]
 subtab, then the menu:Confirm
     Refresh[]
...
==== System Set Manager {gt}Misc{gt}Migrate
...
==== System Set Manager {gt}Misc{gt}Lock/Unlock
...
==== System Set Manager {gt}Misc{gt}Delete
...
==== System Set Manager {gt}Misc{gt}Reboot


Select the appropriate systems, then click the menu:Reboot
     Systems[]
...
As of SUSE Manager 3.1 you can visualize your virtualized, proxy, and system group topologies.
Listed under menu:Systems[Visualization]
 you will find the menu:Virtualization Hierarchy[]
, menu:Proxy
   Hierarchy[]
, and menu:System Grouping[]
 subpages.
...
Click the menu:Toggle Filters[]
 button to open the filters panel.
Systems are filterable by properties such as name, base channel, installed products, etc. 

.
image::systems_visualization_filter_example.png[]


Systems may also be partitioned by checkin time.
Select the check-in date and time and click the menu:Apply[]
 button.
The menu:Reset[]
 button will revert current partition coniguration. 
...
, and click the menu:Search[]
 button.
To list all systems that do _not_ match the criteria, select the menu:Invert
   Result[]
 check box. 
...
Users with the Activation Key Administrator role (including {susemgr} Administrators) can generate activation keys in the {susemgr} Web interface.
With such an activation key, register a {sle} or Red Hat Enterprise Linux system, entitle the system to a {susemgr} service level and subscribe the system to specific channels and system groups through the `rhnreg_ks` command line utility. 

[NOTE]
====
System-specific activation keys created through the menu:Reactivation[]
 subtab of the menu:System
    Details[]
...
.Procedure: Creating Activation Keys....... Select menu:Systems[Activation Keys] from the left bar. 
...
....... menu:Key[] -- Either choose automatic generation by leaving this field blank or enter the key you want to generate in the menu:Key[] field. This string of characters can then be used with `rhnreg_ks` to register client systems with {susemgr}. Refer to <<_s3_sm_system_keys_multiple>> for details. 
+
...
## 2011-03-24 - ke: bwiedemann 20110210: # wirklich alle? auch "'\ und
        0 bytes? Also see the quick starts! 2016-02-17 - moio: ," are not allowed {lt}> (){} will
        get removed automatically #
 Do not insert commas or double quotes in the key.
All other characters are allowed, but `{lt}> (){}` (this includes the space) will get removed automatically.
...
+ 
Selecting `{susemgr} Default` allows client systems to register with the {suse}-provided default channel that corresponds with their installed version of {sle}. You can also associate the key with a custom base channel.
If a system using this key is not compatible with the selected channel, it will fall back to the {susemgr} default channel. 
...
To change the settings of a key, click the key's description in the list to display its menu:Details[]
 page (see <<_fig_s1_sm_systems_akey_details>>
). Via additional tabs you can select child channels, packages, configuration channels, group membership and view activated systems.
Modify the appropriate tab then click the menu:Update Activation Key[]
 button.
To disassociate channels and groups from a key, deselect them in the respective menus by kbd:[]
...
The Client Tools channel should be selected in the menu:Child
    Channels[]
 tab. 

After creating the activation key, you can see in the menu:Details[]
 tab a check box named menu:Configuration
    File Deployment[]
.
...
Click the menu:Update Activation
    Keys[]
...
Separate keys with a comma at the command line with `rhnreg_ks` or in a {kickstart} profile in the menu:Activation Keys[]
...
{susemgr} Provisioning customers can create package profiles via the menu:System Details[]
 page. 
...
, click menu:Create System
   Profile[]
.
Enter a menu:Profile Name[]
 and menu:Profile Description[]
, then click menu:Create
   Profile[]
.
These profiles are displayed on the menu:Stored
   Profiles[]
...
To view software associated with the profile, click the menu:Packages[]
 subtab.
To remove the profile entirely, click menu:Delete
   Profile[]
 at the upper-right corner of the page. 

For more information about stored profiles, see <<_s1_package_profiles>>
...
{susemgr} customers may include completely customizable information about their systems. 
...
When the key exists, you may assign a value to it through the menu:Custom Info[]
 ...
 page.
Refer to <<_s5_sm_system_details_info>>
 for instructions. 

:leveloffset: 1
include::reference-webui-systems-autoinstallation.adoc[]
:leveloffset: 0
...
Third party hypervisors and hypervisor managers such as VMWare vCenter are called "`Virtual Host Managers`"
 (VHM) within {susemgr}. These managers can manage one or multiple virtual hosts, which in turn may contain virtual guests.
{susemgr} ships with a tool called `virtual-host-gatherer` that can connect to VHMs using their API, and request information about virtual hosts.
...
Proceed to menu:Systems[Virtual Host
   Managers]
 page in the WebUI to begin working with a Virtual Host Manager.
In the upper right you can select either menu:Add
   VMware-based Virtual Host Manager[]
 or menu:Add File-based
...
Click menu:Add VMware-based Virtual Host Manager[]
 or menu:Add File-based Virtual Host Manager[]
 to add a Virtual Host Manager. 

Select menu:VMware-based Virtual Host Manager[]
 to enter the location of your VMware-based virtual host.
Enter a menu:Label[]
, menu:Hostname[]
, menu:Port[]
, menu:Username[]
 and menu:Password[]
.
Finally click the menu:Add Virtual Host
    Manager[]
 button.
...
Select menu:File Based Virtual Host
    Manager[]
, then enter a label and URL leading to the location of this file for the ``virtual-host-gatherer``. 
